...
[![branch](https://img.shields.io/badge/branch-Vehicle.Body.Lights.Beam-1D4ED8)](https://covesa.github.io/vehicle_signal_specification/)
...
[![package](https://img.shields.io/badge/package-protobuf.covesa.vss.exterior.beam.v1-444)](v1/)
...
flowchart LR
  ...
  R["Beam<br/><code>…/beams/{beam}</code>"]
...
A beam as this API returns it:

```json
{
  "name": "vehicles/wvwzzz1jz3w000001/beams/beam-z35zh41nrjxha0hy",
...
```json
{
  "Vehicle.Body.Lights.Beam.IsOn": true,
  "Vehicle.Body.Lights.Beam.IsDefect": true
}
```
...
Writing to a beam, and the one thing that will fail:

```mermaid
sequenceDiagram
  participant C as Client
  participant A as Beams
...
```http
PATCH /v1/vehicles/wvwzzz1jz3w000001/beams/beam-z35zh41nrjxha0hy?updateMask=isOn
{ "isOn": true }
```
...
Every beam is addressed by a name that alternates collection and identifier,
...
```go
t := resourcename.ResourceTemplate("vehicles/{vehicle}/beams/{beam}")
t.Parse("vehicles/wvwzzz1jz3w000001/beams/beam-z35zh41nrjxha0hy")
// map[vehicle:wvwzzz1jz3w000001 beam:beam-z35zh41nrjxha0hy]
```

```python
t = resourcename.ResourceTemplate("vehicles/{vehicle}/beams/{beam}")
t.parse("vehicles/wvwzzz1jz3w000001/beams/beam-z35zh41nrjxha0hy")
# {'vehicle': 'wvwzzz1jz3w000001', 'beam': 'beam-z35zh41nrjxha0hy'}
```
...
| --- | --- | --- | --- | --- |
| `is_on` | `bool` | — | `Vehicle.Body.Lights.Beam.IsOn` | Indicates if light is on or off. True = On. False = Off. |
...
| Field | Type | Unit | VSS | Description |
| --- | --- | --- | --- | --- |
| `is_defect` | `bool` | — | `Vehicle.Body.Lights.Beam.IsDefect` | Indicates if light is defect. True = Light is defect. False = Light has no defect. |
...
| Field | Type | Notes |
| --- | --- | --- |
| `name` | `string` | `vehicles/{vehicle}/beams/{beam}`, server-assigned |
...
VSS expands this branch across Low, High, so the combination names one beam
...
A deleted beam is still returned by `Get` and hidden from `List` unless
`show_deleted` is set — which is what makes `Undelete` meaningful.

---

<sub>Generated by `just docs` from COVESA VSS `2026-09-02` (`cd4bc50`) and VDM `2026-07-24` (`36bc939`). Do not edit by hand.<br>
Source: [`beam.proto`](v1/beam.proto) · [`service.proto`](v1/service.proto) · [`messages.proto`](v1/messages.proto)</sub>
