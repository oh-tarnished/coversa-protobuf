# ChargingPort

**Package** `protobuf.covesa.vss.propulsion.charging_port.v1`  
**Resource** `vehicles/{vehicle}/chargingPorts/{charging_port}`  
**Shape** child collection — full CRUD plus undelete

> [!IMPORTANT]
> Auto-generated by `sync docs` from the COVESA Vehicle Data Model,
> spec revision `2026-09-02` (`cd4bc50`).
> Do not edit by hand; run `just docs`.

Properties related to a particular charging port available in the vehicle.

## Service `ChargingPorts`

| Method | Request | Response | Summary |
| --- | --- | --- | --- |
| `GetChargingPort` | `GetChargingPortRequest` | `ChargingPort` | Retrieves one ChargingPort. `GET /v1/{name=vehicles/*/chargingPorts/*}` |
| `ListChargingPorts` | `ListChargingPortsRequest` | `ListChargingPortsResponse` | Lists them, paginated per [AIP-158](https://aip.dev/158). `GET /v1/{parent=vehicles/*}/chargingPorts` |
| `CreateChargingPort` | `CreateChargingPortRequest` | `ChargingPort` | Creates one. `POST /v1/{parent=vehicles/*}/chargingPorts` |
| `UpdateChargingPort` | `UpdateChargingPortRequest` | `ChargingPort` | Partial update; only an actuator is writable. `PATCH` |
| `DeleteChargingPort` | `DeleteChargingPortRequest` | `google.protobuf.Empty` | Soft delete; recoverable until `expire_time`. `DELETE` |
| `UndeleteChargingPort` | `UndeleteChargingPortRequest` | `ChargingPort` | Restores a soft-deleted ChargingPort. `POST …:undelete` |

## Messages

### `ChargingPort`

Properties related to a particular charging port available in the vehicle.

Carries the AIP identity and lifecycle fields — `name`, `uid`, `etag` and the four timestamps — ahead of the signals below. Field numbers 8–15 are reserved.

| Field | Type | Behavior | Unit | Description |
| --- | --- | --- | --- | --- |
| `supported_inlet_types` | `repeated SupportedInletTypes` | `OUTPUT_ONLY` | — | A list of the supported (i.e., available) charging inlets in a particular charging port. IEC types refer to IEC 62196, GBT refers to GB/T 20234. |
| `is_flap_open` | `bool` | `OPTIONAL` | — | Status of the charging port flap(s). |
| `is_charging_cable_connected` | `bool` | `OUTPUT_ONLY` | — | Indicates whether a charging cable is physically connected to a particular charging port or not. |
| `is_charging_cable_locked` | `bool` | `OPTIONAL` | — | Is charging cable locked to prevent removal. |

## Enums

### `SupportedInletTypes`

A list of the supported (i.e., available) charging inlets in a particular charging port. IEC types refer to IEC 62196, GBT refers to GB/T 20234.

| Value | Description |
| --- | --- |
| `SUPPORTED_INLET_TYPES_IEC_TYPE_1_AC` | Iec type 1 ac. |
| `SUPPORTED_INLET_TYPES_IEC_TYPE_2_AC` | Iec type 2 ac. |
| `SUPPORTED_INLET_TYPES_IEC_TYPE_3_AC` | Iec type 3 ac. |
| `SUPPORTED_INLET_TYPES_IEC_TYPE_4_DC` | Iec type 4 dc. |
| `SUPPORTED_INLET_TYPES_IEC_TYPE_1_CCS_DC` | Iec type 1 ccs dc. |
| `SUPPORTED_INLET_TYPES_IEC_TYPE_2_CCS_DC` | Iec type 2 ccs dc. |
| `SUPPORTED_INLET_TYPES_TESLA_ROADSTER` | Tesla roadster. |
| `SUPPORTED_INLET_TYPES_TESLA_HPWC` | Tesla hpwc. |
| `SUPPORTED_INLET_TYPES_TESLA_SUPERCHARGER` | Tesla supercharger. |
| `SUPPORTED_INLET_TYPES_GBT_AC` | Gbt ac. |
| `SUPPORTED_INLET_TYPES_GBT_DC` | Gbt dc. |
| `SUPPORTED_INLET_TYPES_OTHER` | Other. |

