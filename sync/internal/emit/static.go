...
// static.go holds the two vocabulary files that are not transcriptions of the
// source model: the option message definitions, and the extension
// declarations that attach them.
//
// They live here rather than as checked-in .proto files so that nothing under
// protobuf/ is maintained by hand -- the same rule the generated packages
// follow. Only their headers vary between runs, carrying the spec revision.

// optionsBody is the payload of options.proto: the four option messages and
// the Element enum they share.
const optionsBody = `// BranchOptions is the payload of the ` + "`" + `branch` + "`" + ` message option.
//
...
// annotationsBody is the payload of annotations.proto: the extension
// declarations that attach the option messages to protobuf descriptors.
const annotationsBody = `// annotations.proto is the single import a VSS package needs from this
// vocabulary:
//
