...
# 	protoc (unknown)
# source: protobuf/covesa/vss/interior/door/v1/service.proto
#
# target: capnp
...
using Messages = import "/protobuf/covesa/vss/interior/door/v1/messages.capnp";

# Doors manages the Door branch of a vehicle.
#
...
interface Doors @0xa95eb9eaec3437ec {
  # Retrieves one Door.
  #
  ...
  listDoors @1 (request :Messages.ListDoorsRequest) -> (response :Messages.ListDoorsResponse);

  # Creates a Door.
  #
  # POST /v1/{parent=vehicles/*}/doors
  # body: the Door itself
  #
  ...
  #
  # Not idempotent. Retrying after a timeout may create a second Door;
  # supply `door_id` if the caller needs retry safety.
  #
  ...
  createDoor @2 (request :Messages.CreateDoorRequest) -> (response :Door.Door);
  ...
  updateDoor @3 (request :Messages.UpdateDoorRequest) -> (response :Door.Door);

  # Deletes a Door.
  #
  # DELETE /v1/{name=vehicles/*/doors/*}
  #
  # Soft delete: the Door is marked deleted, disappears from List unless
  # `show_deleted` is set, and is purged at `expire_time`. Recover it with
  # UndeleteDoor before then.
  #
  ...
  deleteDoor @4 (request :Messages.DeleteDoorRequest) -> ();

  # Restores a soft-deleted Door.
  #
  # POST /v1/{name=vehicles/*/doors/*}:undelete
  #
  # Valid only before `expire_time`; after that the Door is gone and this
  # returns NOT_FOUND. Clears `delete_time` and `expire_time`.
  #
  # NOT_FOUND no such Door, or already purged
  # ALREADY_EXISTS the Door is not deleted
  # PERMISSION_DENIED caller may not restore it
  # AIP-164 standard method.
  undeleteDoor @5 (request :Messages.UndeleteDoorRequest) -> (response :Door.Door);
}
