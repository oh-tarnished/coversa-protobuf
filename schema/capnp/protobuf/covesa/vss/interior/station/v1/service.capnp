...
# 	protoc (unknown)
# source: protobuf/covesa/vss/interior/station/v1/service.proto
#
# target: capnp
...
using Messages = import "/protobuf/covesa/vss/interior/station/v1/messages.capnp";
using Station = import "/protobuf/covesa/vss/interior/station/v1/station.capnp";

# Stations manages the Station branch of a vehicle.
#
...
interface Stations @0x8559ff25d4e8a110 {
  ...
  #
  # NOT_FOUND no such Station
  # INVALID_ARGUMENT name does not match the resource pattern
  # PERMISSION_DENIED caller may not read it
  # AIP-131 standard method.
  getStation @0 (request :Messages.GetStationRequest) -> (response :Station.Station);

  # Lists the stations of one vehicle.
  #
  ...
  listStations @1 (request :Messages.ListStationsRequest) -> (response :Messages.ListStationsResponse);

  # Creates a Station.
  #
  # POST /v1/{parent=vehicles/*}/stations
  # body: the Station itself
  #
  ...
  #
  # Not idempotent. Retrying after a timeout may create a second Station;
  # supply `station_id` if the caller needs retry safety.
  #
  ...
  createStation @2 (request :Messages.CreateStationRequest) -> (response :Station.Station);
  ...
  updateStation @3 (request :Messages.UpdateStationRequest) -> (response :Station.Station);

  # Deletes a Station.
  #
  # DELETE /v1/{name=vehicles/*/stations/*}
  #
  # Soft delete: the Station is marked deleted, disappears from List unless
  # `show_deleted` is set, and is purged at `expire_time`. Recover it with
  # UndeleteStation before then.
  #
  ...
  deleteStation @4 (request :Messages.DeleteStationRequest) -> ();

  # Restores a soft-deleted Station.
  #
  # POST /v1/{name=vehicles/*/stations/*}:undelete
  #
  # Valid only before `expire_time`; after that the Station is gone and this
  # returns NOT_FOUND. Clears `delete_time` and `expire_time`.
  #
  # NOT_FOUND no such Station, or already purged
  # ALREADY_EXISTS the Station is not deleted
  # PERMISSION_DENIED caller may not restore it
  # AIP-164 standard method.
  undeleteStation @5 (request :Messages.UndeleteStationRequest) -> (response :Station.Station);
}
